...
- (void)testSwiftDeclarationsFromDemangledSymbols {
    ...
    XCTAssertEqualObjects([RTBSwift declarationForDemangledSymbol:@"Module.Foo.__allocating_init<A where A: Swift.Equatable>(A) throws -> Module.Foo" typeName:t], @"init<A where A: Swift.Equatable>(A) throws");
...
- (NSString *)foundationImagePath {
    for(uint32_t i = 0; i < _dyld_image_count(); i++) {
        NSString *path = [NSString stringWithUTF8String:_dyld_get_image_name(i)];
        if([[path lastPathComponent] isEqualToString:@"Foundation"] && [path containsString:@"/Foundation.framework/"]) return path;
    }
    return nil;
}

- (void)testSwiftTypesOfAnImage {
    // the Swift structs, enums and protocols of Foundation, from the metadata sections; the overlay is part of Foundation on macOS 12+ and iOS 15+
    NSString *foundationPath = [self foundationImagePath];
    XCTAssertNotNil(foundationPath);
    if([RTBSwiftTypes imageAtPathHasSwiftTypes:foundationPath] == NO) return;
    
    NSArray *types = [RTBSwiftTypes typesInImageAtPath:foundationPath];
    XCTAssertTrue([types count] > 100, @"%lu", (unsigned long)[types count]);
    XCTAssertTrue([types isEqualToArray:[types sortedArrayUsingSelector:@selector(compare:)]]);
    for(RTBSwiftType *type in types) {
        XCTAssertTrue(type.kind != RTBSwiftTypeKindClass, @"%@", type); // the classes are in the class browser
        XCTAssertEqualObjects(type.imagePath, foundationPath);
    }
    XCTAssertEqualObjects([RTBSwiftTypes typesInImageAtPath:@"/nonexistent"], @[]);
    XCTAssertFalse([RTBSwiftTypes imageAtPathHasSwiftTypes:@"/nonexistent"]);
    
    // a struct: fields from the reflection metadata, conformances, members from the symbol table
    RTBSwiftType *date = [RTBSwiftTypes typeNamed:@"Foundation.Date"];
    XCTAssertNotNil(date);
    XCTAssertEqual(date.kind, RTBSwiftTypeKindStruct);
    XCTAssertEqualObjects(date.kindName, @"struct");
    XCTAssertFalse(date.isGeneric);
    XCTAssertEqualObjects([date nodeName], @"struct Foundation.Date");
    XCTAssertEqualObjects([date children], @[]);
    XCTAssertTrue([types containsObject:date]);
    NSString *declaration = [date declaration];
    XCTAssertTrue([declaration hasPrefix:@"/* Generated by RuntimeBrowser\n   Image: "], @"%@", declaration);
    XCTAssertTrue([declaration containsString:@"\nstruct Foundation.Date: "], @"%@", declaration);
    XCTAssertTrue([declaration containsString:@"Swift.Hashable"], @"%@", declaration);
    [self assertHeader:declaration containsLine:@"    var _time: Swift.Double"];
    [self assertHeader:declaration containsLine:@"init(timeIntervalSinceReferenceDate: Swift.Double)"];
    [self assertHeader:declaration containsLine:@"var timeIntervalSinceReferenceDate: Swift.Double { get }"];
    [self assertHeader:declaration containsLine:@"static var distantFuture: Foundation.Date { get }"];
    
    // an enum: cases and payloads from the field descriptor
    RTBSwiftType *strategy = [RTBSwiftTypes typeNamed:@"Foundation.JSONDecoder.DateDecodingStrategy"];
    XCTAssertEqual(strategy.kind, RTBSwiftTypeKindEnum);
    declaration = [strategy declaration];
    [self assertHeader:declaration containsLine:@"enum Foundation.JSONDecoder.DateDecodingStrategy {"];
    [self assertHeader:declaration containsLine:@"    case deferredToDate"];
    [self assertHeader:declaration containsLine:@"    case secondsSince1970"];
    XCTAssertTrue([declaration containsString:@"    case formatted("], @"%@", declaration);
    
    // a generic struct: the parameters are named like the demangler does, the field types are demangled
    RTBSwiftType *measurement = [RTBSwiftTypes typeNamed:@"Foundation.Measurement<A>"];
    XCTAssertNotNil(measurement);
    XCTAssertTrue(measurement.isGeneric);
    declaration = [measurement declaration];
    XCTAssertTrue([declaration containsString:@"\nstruct Foundation.Measurement<A>: "], @"%@", declaration);
    [self assertHeader:declaration containsLine:@"    let unit: A"];
    [self assertHeader:declaration containsLine:@"    var value: Swift.Double"];
    [self assertHeader:declaration containsLine:@"init(value: Swift.Double, unit: A)"];
    
    // a protocol: inherited protocols and associated types
    RTBSwiftType *referenceConvertible = [RTBSwiftTypes typeNamed:@"Foundation.ReferenceConvertible"];
    XCTAssertEqual(referenceConvertible.kind, RTBSwiftTypeKindProtocol);
    declaration = [referenceConvertible declaration];
    XCTAssertTrue([declaration containsString:@"\nprotocol Foundation.ReferenceConvertible: "], @"%@", declaration);
    XCTAssertTrue([declaration containsString:@"Swift.Hashable"], @"%@", declaration);
    [self assertHeader:declaration containsLine:@"    associatedtype ReferenceType"];
    
    XCTAssertNil([RTBSwiftTypes typeNamed:@"Foundation.NoSuchType"]);
}

- (void)testSwiftConformances {
    // the conformance records of the loaded images cover the Objective-C classes with Swift extensions and the Swift classes
    XCTAssertEqualObjects([RTBSwiftTypes conformancesOfClass:Nil], @[]);
    XCTAssertEqualObjects([RTBSwiftTypes conformancesOfClass:[RTBTestClass class]], @[]);
    
    NSArray *conformances = [RTBSwiftTypes conformancesOfClass:[NSObject class]];
    if([conformances count] == 0) return; // no Swift overlay loaded
    XCTAssertTrue([conformances containsObject:@"Swift.Hashable"], @"%@", conformances);
    XCTAssertTrue([conformances containsObject:@"Swift.CustomStringConvertible"], @"%@", conformances);
    XCTAssertTrue([conformances isEqualToArray:[conformances sortedArrayUsingSelector:@selector(compare:)]]);
    XCTAssertEqualObjects([[RTBClass classStubWithClass:[NSObject class]] sortedSwiftConformances], conformances);
    
    NSString *header = [RTBRuntimeHeader headerForClass:[NSObject class] displayPropertiesDefaultValues:NO];
    [self assertHeader:header containsLine:@"// Swift conformances, from the conformance records of the loaded images"];
    [self assertHeader:header containsLine:@"Swift.Hashable"];
    
    // a Swift class, found by its type descriptor
    Class timerPublisher = NSClassFromString(@"_TtCE10FoundationCSo7NSTimer14TimerPublisher");
    if(timerPublisher) {
        XCTAssertEqualObjects([RTBSwiftTypes conformancesOfClass:timerPublisher], (@[@"Combine.ConnectablePublisher", @"Combine.Publisher"]));
    }
}
